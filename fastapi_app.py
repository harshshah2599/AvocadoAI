...
#         analysis = analyze_content_with_llm(clean_text)
...
class Article(BaseModel):
    ...
    Attributes:
    - text (str): The content of the article to be analyzed.
    - other_field (int): An additional field for demonstration purposes (if used).
    """
    text: str
...
def analyze_article(article: Article):
    ...
    try:
        # Perform content analysis using the provided function
        analysis = analyze_content_with_llm(clean_text)
        return {"analysis": analysis}  # Ensure analysis is serializable
    except Exception as e:
        # Log the exception and return a 500 error response
        raise HTTPException(status_code=500, detail=str(e))
